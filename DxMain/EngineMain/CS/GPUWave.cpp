...
void GPUWave::BuildResources(ID3D12GraphicsCommandList* cmdList)
{
	...
	ThrowIfFailed(md3dDevice->CreateCommittedResource(
		...
		D3D12_RESOURCE_STATE_GENERIC_READ,
		nullptr,
		IID_PPV_ARGS(mNextUploadBuffer.GetAddressOf())));
	...
	cmdList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(mNextSol.Get(),
		D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_STATE_COPY_DEST));
	UpdateSubresources(cmdList, mNextSol.Get(), mNextUploadBuffer.Get(), 0, 0, num2DSubresources, &subResourceData);
	cmdList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(mNextSol.Get(),
		D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_GENERIC_READ));
}
...
void GPUWave::Update(const GameTimer& gt, ID3D12GraphicsCommandList* cmdList, ID3D12RootSignature* rootSig, ID3D12PipelineState* pso)
{
	...
	if (t >= mTimeStep)
	{
		...
		UINT numGroupsX = mNumColumns / 16;
