...
void LitColumnsApp::BuildShadersAndInputlayout()
{
	...
	mShaders["standardVS"] = d3dUtil::CompileShader(L"shaders\\LitWave_Default.hlsl", nullptr, "VS", "vs_5_1");
	mShaders["opaquePS"] = d3dUtil::CompileShader(L"shaders\\LitWave_Default.hlsl", nullptr, "PS", "ps_5_1");
...
void LitColumnsApp::BuildSkullGeometry()
{
	...
	std::vector<Vertex_LitWave> vertices(vcount);
	...
	const UINT vbByteSize = (UINT)vertices.size() * sizeof(Vertex_LitWave);
...
void LitColumnsApp::BuildFrameResrouces()
{
	for (int i =0; i< gNumFrameResources_LitColumns; i++)
	{
		mFrameResources.push_back(std::make_unique<FrameResource_LitWave>(md3dDevice.Get(), 1, (UINT)mAllRitems.size(), (UINT)mMaterials.size(), 0));
	}
}
...
{
	auto boxRitem = std::make_unique<RenderItem_LitColumns>();
	...
	auto gridRitem = std::make_unique<RenderItem_LitColumns>();
	...
	auto skullRitem = std::make_unique<RenderItem_LitColumns>();
	...
	for (int i = 0; i < 5; ++i)
	{
		auto leftCylRitem = std::make_unique<RenderItem_LitColumns>();
		auto rightCylRitem = std::make_unique<RenderItem_LitColumns>();
		auto leftSphereRitem = std::make_unique<RenderItem_LitColumns>();
		auto rightSphereRitem = std::make_unique<RenderItem_LitColumns>();
...
void LitColumnsApp::Update(const GameTimer& gt)
{
	...
	mCurrentFrameResourceIndex = (mCurrentFrameResourceIndex + 1) % gNumFrameResources_LitColumns;
...
void LitColumnsApp::UpdateObjectCBs(const GameTimer& gt)
{
	...
	for (auto& e : mAllRitems)
	{
		...
		if (e->NumFramesDirty > 0)
		{
			...
			//XMStoreFloat4x4(&objConstants., XMMatrixTranspose(texTransform));
...
void LitColumnsApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem_LitColumns *> ritems)
{
