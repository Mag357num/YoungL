#include "stdafx.h"
#include "CameraAndDynamicIndexApp.h"
...
void CameraAndDynamicIndexingApp::UpdateObjectCBs(const GameTimer& gt)
{
	...
	for (auto& e : mAllRitems)
	{
		...
		if (e->NumFramesDirty > 0)
		{
			...
			ObjectConstants_CameraDyIndex objConstants;
...
void CameraAndDynamicIndexingApp::UpdateMaterialBuffer(const GameTimer& gt)
{
	...
	for (auto& e : mMaterials)
	{
		...
		if (mat->NumFramesDirty > 0)
		{
			...
			MaterialData_CameraDyindex matData;
...
void CameraAndDynamicIndexingApp::BuildShadersAndInputLayout()
{
	...
	mShaders["standardVS"] = d3dUtil::CompileShader(L"shaders\\CameraDyIndex_Default.hlsl", nullptr, "VS", "vs_5_1");
	mShaders["opaquePS"] = d3dUtil::CompileShader(L"shaders\\CameraDyIndex_Default.hlsl", nullptr, "PS", "ps_5_1");
...
void CameraAndDynamicIndexingApp::BuildShapeGeometry()
{
	...
	std::vector<Vertex_CameraDyIndex> vertices(totalVertexCount);
	...
	const UINT vbByteSize = (UINT)vertices.size() * sizeof(Vertex_CameraDyIndex);
	...
	geo->VertextByteStride = sizeof(Vertex_CameraDyIndex);
...
void CameraAndDynamicIndexingApp::BuildFrameResources()
{
	...
	{
		mFrameResources.push_back(std::make_unique<FrameResource_CameraDyIndex>(md3dDevice.Get(),
...
{
	auto boxRitem = std::make_unique<RenderItem_CameraDyIndex>();
	...
	auto gridRitem = std::make_unique<RenderItem_CameraDyIndex>();
	...
	for (int i = 0; i < 5; ++i)
	{
		auto leftCylRitem = std::make_unique<RenderItem_CameraDyIndex>();
		auto rightCylRitem = std::make_unique<RenderItem_CameraDyIndex>();
		auto leftSphereRitem = std::make_unique<RenderItem_CameraDyIndex>();
		auto rightSphereRitem = std::make_unique<RenderItem_CameraDyIndex>();
...
void CameraAndDynamicIndexingApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem_CameraDyIndex*>& ritems)
{
	UINT objCBByteSize = d3dUtil::CalcConstantBufferByteSize(sizeof(ObjectConstants_CameraDyIndex));
