...
const int gNumFrameResources_LitWave = 3;
...
struct RenderItem_LitWave
{
	...
	//thus ,when we modify object data we should set numFramesDirty =gNumFrameResources_LitWave so that each frame resource gets the update.
	int NumFramesDirty = gNumFrameResources_LitWave;
...
private:
	...
	std::unordered_map<std::string, ComPtr<ID3DBlob>> mPSOs;
	...
	std::vector<RenderItem_LitWave*> mRitemLayer[(int)RenderLayer::Count];
